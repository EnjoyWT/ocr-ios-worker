...
final class iOSVisionService: Sendable {
    enum OCRError: Error {
    ...
    static let shared = iOSVisionService()
    private let logger = Logger(label: "ios-vision")
    
    private init() {}
    
    func recognizeText(from imageData: Data, language: String? = nil, confidence: Float? = nil, deviceId: String) async throws -> OCRResponse.OCRData {
        let startTime = CFAbsoluteTimeGetCurrent()
        
        logger.info("Starting iOS OCR processing", metadata: [
            "deviceId": "\(deviceId)",
        ...
        return try await withCheckedThrowingContinuation { continuation in
            let request = VNRecognizeTextRequest { request, error in
                ...
                self.logger.info("iOS OCR processing completed", metadata: [
                    "deviceId": "\(deviceId)",
